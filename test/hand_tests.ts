...
describe("Tests that will evaluate the options from a Blackjack hand", function () {
  ...
  describe("Does my hand have an ace?", () => {
    it("Should return true when my hand has an ace and false when it doesn't", () => {
      let card1: Card = masterDeck.getCard(CardFace.ACE, Suit.CLUBS);
      let card2: Card = masterDeck.getCard(CardFace.KING, Suit.CLUBS);
      let hand: BlackjackHand = new BlackjackHand().AddCards(new Array<Card>(card1, card2));
      expect(hand.HasAce()).to.equal(true);

      let card3: Card = masterDeck.getCard(CardFace.FIVE, Suit.CLUBS);
      let card4: Card = masterDeck.getCard(CardFace.KING, Suit.CLUBS);
      let hand2: BlackjackHand = new BlackjackHand().AddCards(new Array<Card>(card3, card4));
      expect(hand2.HasAce()).to.equal(false);
    });
